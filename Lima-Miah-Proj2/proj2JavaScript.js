...
function start() {
    ...
    randomNumber = tileID.sort(function () {
...
function doClick(x) {
    ...
    if (x.id != empty + '') {
        ...
        if ((Math.abs(emptyI - selectedI) == 1 && emptyJ == selectedJ) ||
            ...
            document.getElementById("moves").innerHTML = "Number of moves thus far: " + Num_moves;

            if (isDone()) {
                check_Win = true;
                document.getElementById("moves").innerHTML = "CONGRATULATIONS!  Number of moves it took to complete: " + Num_moves;
                document.getElementById("time").innerHTML = "Time it took to complete the game: " + time;
...
function updateTime() {
    ++time;
    document.getElementById("time").innerHTML = "Amount spent on current game in seconds: " + time;
}
